...
def check_key(keyid):
	...
	try:
		...
		if kstatus == "Enabled" or kstatus == "Disabled":
			...
			common.log_warning("WARNING: key is not valid or is managed by AWS")
			return False
...
def aws_common(type,t1,tt1,tt2,flag1,flag2):
    ...
    try:
        ...
        if tt1=="bucket" or tt1=="s3_bucket_name":
            if context.debug5: log.debug("DEBUG5: aws_common: type=%s tt1=%s tt2=%s", type, tt1, tt2)
            if type != "aws_s3_bucket":
                if "." not in tt2:
                    if tt2 != "" and tt2 !="null":
                        ...
                        common.log_warning("WARNING: bucket name from arn " +tt2 +" is empty or null")
                        return skip,t1,flag1,flag2

                    
                    
        if tt1.endswith("bucket_arn"):
            if context.debug5: log.debug("DEBUG5: aws_common: bucket_name=%s lhs=%s", tt2, tt1)
            if tt2.startswith("arn:aws:s3"):
                tt2=tt2.split(":")[-1]
                if context.debug5: 
                    print("DEBUG5: aws_common: bucket_name=", tt2)
           ...
           if tt2 != "null":
                try:
                    if context.sglist[tt2]:
                        ...
                        common.log_warning("WARNING: security group not found in sglist %s",  tt2)
                except KeyError as e:
                    common.log_warning("WARNING: subnet_id not found in subnet list %s",  tt2)
            ...
            if tt2 != "null":
                if tt2.startswith('vpc-'): 
                    if tt2 in context.vpclist:
                        ...
                        common.log_warning("WARNING: vpc_id not found in vpclist %s", tt2)
            ...
            if tt2 != "null":     
                ...
                try:
                    ...
                    common.log_warning("WARNING: lambda_function_arn not found in lambda list %s", tt2)
                    return skip,t1,flag1,flag2
                ...
                if "/" not in tt2:
                    try:
                        if context.rolelist[tt2] and not common.ref_skipped("aws_iam_role", tt2):
                            ...
                            common.log_warning("WARNING: role not found in rolelist %s",  tt2)
                    except KeyError as e:
                        common.log_warning("WARNING: role not found in rolelist [ke] %s",  tt2)
