...
def aws_iam_role(t1,tt1,tt2,flag1,flag2):
	...
	if tt1 == "name":
		...
		if "arn:aws:iam::aws:policy" not in tt2:
			pn=tt2.split("/")[-1]
			if common.ref_skipped("aws_iam_policy", pn):
				# boundary policy was skipped/excluded - drop the optional attribute
				# (the literal ARN cannot be kept; it may be an invalid/partner ARN)
				skip=1
			else:
